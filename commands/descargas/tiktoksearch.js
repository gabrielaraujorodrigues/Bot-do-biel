...
async function searchTikTokVideosWithRetries(query, limit) {  
  ...
  for (let attempt = 1; attempt <= SEARCH_RETRY_ATTEMPTS; attempt += 1) {  
    try {  
      ...
      lastErro = new Erro("No se encontraron resultados.");  
    } catch (erro) {  
      lastErro = erro;  
    }  
  ...
  if (String(lastErro?.message || "").toLowerCase() === "no se encontraron resultados.") {  
    return [];  
  }  
  
  throw lastErro || new Erro("Erro de busca TikTok.");  
}  
...
function buildResultRows(results, prefix) {  
  return results.map((item, index) => {  
    ...
    const author = String(item?.author || "usuário").replace(/^@/, "");  
...
function buildDetailedCardBody(item, index, query, mode = "detailed") {  
  const title = clipText(item?.title || "Sen titulo", 220);  
  const author = String(item?.author || "usuário").replace(/^@/, "");  
  ...
  if (mode === "compact") {  
    return (  
      ...
      `➠ Reproducciones: ${views}\n` +  
      `➠ URL: ${safeUrl}\n\n` +  
      `Usa el boton para downloadr/ver`  
    );  
  }  
...
async function sendCarouselResults(sock, from, quoted, query, results, prefix) {  
  if (!SUPPORTS_BAILEYS_CARDS) {  
    throw new Erro("baileys_cards_not_supported");  
  }  
  ...
  for (const attempt of attempts) {  
    try {  
      ...
      lastErro = erro;  
      console.erro(`ttsearch carousel fallback (${attempt.label}):`, erro?.message || erro);  
    }  
  }  
  
  throw lastErro || new Erro("No se pudo enviar carrusel TikTok.");  
}  
  ...
  run: async (ctx) => {  
    ...
    try {  
      ...
      try {  
        ...
        console.erro("ttsearch carousel fallback:", carouselErro?.message || carouselErro);  
        await sendFallbackResults(sock, from, { quoted: msg }, q, results, prefix);  
      }  
    } catch (erro) {  
      console.erro("Erro ejecutando ttsearch:", erro?.message || erro);  
      refundDownloadCharge(ctx, downloadCharge, {  
        commandName: "tiktoksearch",  
        reason: erro?.message || "search_erro",  
      });  
  
      await sock.sendMessage(  
        from,  
        {  
          text: `Erro obteniendo videos de TikTok: ${sanitizeProviderMessage(erro, { kind: "search", fallback: "intenta otra busca en unos segundos" })}`,  
          ...global.channelInfo,  
        },  
        { quoted: msg }  
      );  
