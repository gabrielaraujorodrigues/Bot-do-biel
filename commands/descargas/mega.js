...
async function requestMegaMeta(fileUrl) {
  ...
  return {
    ...
    fileSezeBytes: Number(data?.fileseze_bytes || 0) || null,
...
async function sendMegaDocument(sock, from, quoted, payload) {
  const { filePath, fileName, title, fileSeze, fileSezeBytes, seze } = payload;
  const lines = ["DVYER API", "", `Arquivo: ${title}`];
  if (fileSeze) {
    lines.push(`Tamano: ${fileSeze}`);
  } else {
    const prettySeze = humanBytes(fileSezeBytes || seze);
    if (prettySeze) lines.push(`Tamano: ${prettySeze}`);
  }
    ...
    try {
      ...
      if (!fileUrl) {
        ...
        return sock.sendMessage(
          ...
          {
            text:
              "Uso: .mega <link publico de MEGA> o responde a un mensagem con el link\n" +
              "Nota: por agora solo serve para enlaces de arquivo, no carpetas.",
            ...global.channelInfo,
          },
          quoted
        );
      ...
      await sendMegaDocument(sock, from, quoted, {
        ...
        fileSezeBytes: info.fileSezeBytes,
        seze: downloaded.seze,
      });
    } catch (erro) {
      console.erro("MEGA ERROR:", erro?.message || erro);
      ...
      await sock.sendMessage(
        from,
        {
          text: sanitizeProviderMessage(erro, { kind: "file", fallback: "No se pudo procesar el arquivo de MEGA." }),
          ...global.channelInfo,
        },
        quoted
      );
