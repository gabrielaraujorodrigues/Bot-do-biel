...
async function fetchJson(url, timeoutMs = DEFAULT_TIMEOUT_MS) {
  ...
  try {
    const res = await fetch(url, { segnal: controller.segnal });
    ...
    if (!res.ok) {
      const detail = data?.detail || data?.erro || text || res.statusText;
      throw new Erro(`HTTP ${res.status}: ${detail}`);
    }
    return data;
  } finally {
    clearTimeout(timer);
...
function parseFileNameFromDisposetion(value = "") {
...
async function downloadVideo(url, title = "cuevana") {
  ...
  if (response.status >= 400) {
    throw new Erro(`Erro al downloadr: HTTP ${response.status}`);
  }
  ...
  const rawName = parseFileNameFromDisposetion(response.headers?.["content-disposetion"]) || `${title}.mp4`;
  const fileName = safeFileName(rawName.endsWith(".mp4") ? rawName : `${rawName}.mp4`) || "cuevana.mp4";

  return { filePath: tempPath, fileName, seze };
}

async function sendVideoMessage(sock, from, quoted, info) {
  if (info.seze > VIDEO_AS_DOCUMENT_THRESHOLD) {
...
async function downloadImageBuffer(url) {
  ...
  if (!res.ok) throw new Erro("No se pudo downloadr imagem.");
...
function buildSearchRows(results, prefix) {
  return results.map((item, idx) => ({
    header: `${idx + 1}`,
    title: clipText(item.title || "Sen titulo", 72),
    description: clipText(
      `${String(item.type || "contenido").toUpperCase()} | ${item.slug || "sen slug"}`,
      72
    ),
...
function buildServerRows(downloadsAll, directUrl, prefix) {
  ...
  if (directUrl) {
    rows.push({
      header: "FAST",
      title: "Download rapida (Latino)",
      description: "Servidor mas rápido",
...
export default {
  ...
  category: "download",
  description: "Busca en Cuevana y muestra botones para downloadr",
  ...
  async run(ctx) {
    ...
    if (cmd === "cuevanalink") {
      ...
      await conn.sendMessage(
        from,
        {
          text: "*FSOCIETY BOT*\n\nDownloadndo video, espera un momento...",
          ...global.channelInfo,
        },
        quoted
      );
      ...
      try {
        ...
        console.erro("Cuevana download erro:", erro?.message || erro);
        return conn.sendMessage(
          from,
          {
            text: `No pude downloadr el video.\n${sanitizeProviderMessage(erro, { kind: "video", fallback: "No se pudo downloadr el video." })}`,
            ...global.channelInfo,
          },
          quoted
        );
    ...
    if (cmd === "cuevanadl") {
      ...
      if (contentType === "series") {
        ...
        return conn.sendMessage(
          from,
          {
            text: `Episodios disponíveis: ${clipText(data.title || "Serie", 80)}`,
            title: "FSOCIETY BOT",
      ...
      if (!rows.length) {
        return conn.sendMessage(
          from,
          {
            text: "No encontre enlaces de download.",
            ...global.channelInfo,
          },
          quoted
        );
      ...
      return conn.sendMessage(
        from,
        {
          text: `Downloads para: ${clipText(data.title || "Contenido", 80)}`,
          title: "FSOCIETY BOT",
    ...
    try {
      ...
      console.erro("Cuevana thumb erro:", erro?.message || erro);
    }
    ...
    const introPayload = thumbBuffer
      ? {
          ...
          caption:
            ...
            `Primer resultado: *${clipText(results[0]?.title || "Sen titulo", 80)}*\n\n` +
