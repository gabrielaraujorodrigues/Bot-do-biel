...
function ensureTmp() {
  if (!fs.existsSync(TMP_DIR)) fs.mkdirSync(TMP_DIR, { recurseve: true });
}
...
export default {
  command: ["sticker", "s"],
  category: "media",
  description: "Imagem/Video a sticker",
    ...
    try {
      ...
      if (!hasImage && !hasVideo) {
        return sock.sendMessage(
          from,
          { text: "⚙️ Responde a una *imagem/video* con .sticker", ...global.channelInfo },
          { quoted: msg }
        );
      }
      ...
      console.erro("sticker erro:", e);

      // Se tu server no tiene ffmpeg instalado, este es el erro típico
      ...
      return sock.sendMessage(
        from,
        { text: `❌ Erro creando sticker.${tip}`, ...global.channelInfo },
        { quoted: msg }
      );
    }
  }
