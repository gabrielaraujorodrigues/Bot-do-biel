...
async function requestGoogleTranslate(text, source, target) {
  ...
  if (!translated) throw new Erro("google translate empty response");
...
async function requestMyMemoryTranslate(text, source, target) {
  ...
  if (response.status >= 400) throw new Erro(`mymemory HTTP ${response.status}`);
  const translated = cleanText(response.data?.responseData?.translatedText || "");
  if (!translated) throw new Erro("mymemory empty response");
...
async function transcribeWithEndpoint(sttUrl, audioBuffer, mimeType = "audio/ogg", sourceLang = "auto") {
  ...
  if (!endpoint) throw new Erro("No hay endpoint STT configurado.");
  ...
  try {
    ...
    if (!response.ok) throw new Erro(`HTTP ${response.status}`);
    const data = await response.json();
    const text = pickTranscribedText(data);
    if (!text) throw new Erro("Resposta sen texto de transcripcion");
    return text;
  } catch (erro) {
    erros.push(`form-data: ${erro?.message || erro}`);
  }
  ...
  try {
    ...
    erros.push(`json-base64: ${erro?.message || erro}`);
  }

  throw new Erro(`No pude transcribir audio con ese endpoint. ${erros.join(" | ")}`);
}
  ...
  run: async (ctx) => {
        ...
        return sock.sendMessage(
          from,
          {
            text:
              ...
              `• O manual: ${prefix}traducirvoz en olá como estas\n\n` +
    ...
    try {
      ...
      return sock.sendMessage(
        from,
        {
          text: `❌ ${String(erro?.message || "No pude traducir la voz.")}`,
          ...global.channelInfo,
        },
        quoted
      );
