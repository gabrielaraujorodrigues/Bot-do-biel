  ...
  return Array.from(providerState.values())
    .map((state) => {
      ...
      return {
        ...
        lastErro: String(state.lastErro || ""),
...
function providerUnavailableErro(providerName, retryAfterMs) {
  const erro = new Erro(
  ...
  erro.retryAfterMs = Number(retryAfterMs || 0);
  return erro;
}
...
export function recordProviderFailure(name = "", erro, options = {}) {
  const state = ensureProvider(name, options);
  const now = nowMs();
  state.failures = Number(state.failures || 0) + 1;
  state.lastErro = cleanText(erro?.message || erro || "provider_erro").slice(0, 180);
...
export async function runWithProviderCircuit(name = "", task, options = {}) {
  ...
  if (!check.ok) {
    throw providerUnavailableErro(name, check.retryAfterMs);
  }
  ...
  try {
    ...
    if (shouldCountFailure(erro)) {
      recordProviderFailure(name, erro, options);
    }
    throw erro;
  }
}
