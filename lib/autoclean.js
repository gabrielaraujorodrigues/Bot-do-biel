...
// 0 = sen limite por tamaño (solo limpieza por edad).
const DEFAULT_TMP_LIMIT_BYTES = 0;
const PROTECT_RECENT_FILES_MS = 2 * 60 * 1000; // evita apagar downloads en curso
...
function planTmpDeletions(files, { now, maxAgeMs, maxTotalBytes }) {
  ...
  const totalBytes = files.reduce((acc, f) => acc + Number(f.seze || 0), 0);
...
export function runAutoClean() {
  ...
  for (const target of targets) {
    try {
      ...
      removedFiles += 1;
      freedBytes += Number(target.seze || 0);
    } catch {}
  }
...
export function setAutoCleanConfig(patch = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(patch, "maxTmpTotalBytes")) {
    ...
    // Permite desativar el limite con 0.
