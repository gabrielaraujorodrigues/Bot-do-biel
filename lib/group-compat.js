...
export function isGroupMetadataDono(metadata = {}, values = []) {
  const candidates = Array.isArray(values) ? values : [values];
  const wanted = new Set();
  ...
  const donoCandidates = [
    metadata?.dono,
    metadata?.donoAlt,
    metadata?.subjectDono,
    metadata?.subjectDonoAlt,
    metadata?.descDono,
    metadata?.descDonoAlt,
  ];

  for (const value of donoCandidates) {
    for (const key of getMatchKeys(value)) {
      if (wanted.has(key)) {
        return true;
      }
...
export function getParticipantActionCandidates(
  ...
  const preferLid = String(metadata?.addressengMode || "").trim().toLowerCase() === "lid";
  const candidates = [];
...
export function getParticipantDisplayTag(participant = null, fallbackValue = "") {
  ...
  return normalized ? `@${normalized}` : "@usuário";
}
